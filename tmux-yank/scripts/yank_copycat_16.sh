...
. "${TMUX_PLUGIN_MANAGER_PATH}/tmux-yank/scripts/vars.sh"
. "${TMUX_PLUGIN_MANAGER_PATH}/tmux-yank/scripts/helpers.sh"
...
if clipboard_cmd="$(_clipboard_cmd_helper)"; then
    tmux bind-key -n "${yank_key}" run "tmux send-keys Enter;
        ...
        ${TMUX_PLUGIN_MANAGER_PATH}/tmux-copycat/scripts/copycat_mode_quit.sh;
        tmux unbind-key -n ${yank_key};"

    tmux bind-key -n "${put_key}" run "tmux send-keys Enter;
        tmux paste-buffer; ${TMUX_PLUGIN_MANAGER_PATH}/tmux-copycat/scripts/copycat_mode_quit.sh;
    ...
    tmux bind-key -n q run "tmux unbind-key -n ${yank_key}; tmux unbind-key -n ${put_key};
        tmux unbind-key -n ${yank_put_key}; ${TMUX_PLUGIN_MANAGER_PATH}/tmux-copycat/scripts/copycat_mode_quit.sh;
        tmux send-keys q";
